...
if (!function_exists('getOption')) {
    /**
     * Read an option value from config cache or database.
     *
     * Table: settings(option_key, option_value, tenant_id)
     */
    function getOption(string $key, $default = null)
    {
        // Prefer config cache (some apps preload all settings into config('settings'))
        try {
            ...
            // ignore - config helper may not be ready very early in bootstrap
        }

        // Fallback to DB (Eloquent model is OK after app boot)
        try {
            $q = \App\Models\Setting::query()->where('option_key', $key);

            // If tenant mode is used, prefer current tenant (when available)
            if (function_exists('tenant') && ($tenant = tenant())) {
                $q->where(function ($sub) use ($tenant) {
                    $sub->whereNull('tenant_id')->orWhere('tenant_id', $tenant->id);
                });
            }

            $val = $q->value('option_value');
...
{
    // Resolve setting -> file_managers.id -> file path
    if (!$option_key) {
        return asset('assets/images/no-image.jpg');
    }

    $tenantId = getTenantId();
    $setting = Setting::where('tenant_id', $tenantId)->where('option_key', $option_key)->first();

    if (isset($setting->option_value) && $setting->option_value !== null && $setting->option_value !== '') {
        // option_value stores FileManager ID (numeric)
        $file = FileManager::where('tenant_id', $tenantId)->select('path', 'storage_type')->find($setting->option_value);

        if (!is_null($file) && isset($file->path) && $file->path) {
            $disksToTry = [];

            if (isset($file->storage_type) && $file->storage_type) {
                $disksToTry[] = $file->storage_type;
            }

            // Fallback disks (env/config mismatches)
            $envDisk = config('app.STORAGE_DRIVER');
            if ($envDisk) $disksToTry[] = $envDisk;

            $defaultDisk = config('filesystems.default');
            if ($defaultDisk) $disksToTry[] = $defaultDisk;

            $disksToTry = array_values(array_unique(array_filter($disksToTry)));
    ...
    // Default fallbacks per key (restores hero/banner background when not configured)
    $defaults = [
        // Frontend breadcrumb/hero background
        'page_breadcrumb' => 'assets/images/branding/alumni-center2.png',

        // If admin pages use these keys and no setting exists, fall back gracefully
        'banner_background_breadcrumb' => 'zaifiles/assets/images/breadcrumb.jpg',
        'about_us_background_breadcrumb' => 'assets/images/branding/alumni-center2.png',
        'upcoming_events_background' => 'assets/images/branding/alumni-center2.png',

        // Logos (only if no uploaded setting exists)
        'app_logo' => 'assets/images/branding/ksu-alumni-logo.png',
        'app_black_logo' => 'assets/images/branding/ksu-alumni-logo.png',
        'app_fav_icon' => 'assets/images/branding/ksu-logo.png',
        'app_preloader' => 'assets/images/branding/ksu-logo.png',
        'login_left_image' => 'assets/images/branding/alumni-center2.png',
    ];

    if (array_key_exists($option_key, $defaults)) {
        return asset($defaults[$option_key]);
    }
...
{
    // SaaS central domain logic
    if (isCentralDomain()) {

        // If SaaS addon is installed, tenant comes from logged-in user (if any)
        if (isAddonInstalled('ALUSAAS')) {
            return auth()->user()?->tenant_id; // can be null when not logged in
        }

        // Non-SaaS: use first domain if exists
        $domain = \Stancl\Tenancy\Database\Models\Domain::first();
        return $domain?->tenant_id; // SAFE (null if no domain row yet)
    }

    // Tenant context logic
    if (isAddonInstalled('ALUSAAS')) {
        // stancl helper: tenant('id') may be null if tenancy not initialized
        return tenant('id');
    }

    // Non-SaaS: use first domain if exists
    $domain = \Stancl\Tenancy\Database\Models\Domain::first();
    return $domain?->tenant_id; // SAFE
}
