...
class SettingController extends Controller
{
    ...
    {
        // KSU build: configuration forms must be available (remove demo restriction).
    ...
    {
        // KSU build: allow configuration updates (remove demo restriction).
        try {
    ...
    public function registrationFormSetting()
    {
        $data['title'] = __('Registration Form Setting');
        $data['showManageApplicationSetting'] = 'show';
        $data['activeRegFormSetting'] = 'active';
        $data['regForm'] = RegisterForm::where(['tenant_id' => getTenantId()])->first();
        return view('admin.setting.general_settings.registration-form-settings', $data);
    }

    public function registrationFormSettingStore(Request $request)
    {
        RegisterForm::updateOrCreate([
            'tenant_id' => getTenantId()
        ],[
            'enable_batch' => $request->enable_batch ? 1 : 0,
            'enable_department' => $request->enable_department ? 1 : 0,
            'enable_passing_year' => $request->enable_passing_year ? 1 : 0,
            'enable_role_number' => $request->enable_role_number ? 1 : 0,
            'enable_attachment' => $request->enable_attachment ? 1 : 0,
            'enable_date_of_birth' => $request->enable_date_of_birth ? 1 : 0,
            'enable_gender' => $request->enable_gender ? 1 : 0,
