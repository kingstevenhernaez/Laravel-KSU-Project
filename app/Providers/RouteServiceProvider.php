...
class RouteServiceProvider extends ServiceProvider
{
    ...
    protected function mapWebRoutes()
    {
        foreach ($this->centralDomains() as $domain) {
            ...
            if (function_exists('isAddonInstalled') && isAddonInstalled('ALUDONATION')) {
                //donation routes
                Route::domain($domain)
                    ->middleware(['web', 'auth', 'admin', 'is_email_verify', '2fa_verify'])
                    ->namespace($this->namespace)
            ...
            if (function_exists('isAddonInstalled') && isAddonInstalled('ALUCOMMITTEE')) {
                //committee routes
                Route::domain($domain)
                    ->middleware(['web', 'auth', 'admin', 'is_email_verify', '2fa_verify'])
                    ->namespace($this->namespace)
    ...
    public function boot()
    {
        ...
        $this->routes(function () {
            if (function_exists('isAddonInstalled') && function_exists('isCentralDomain') && isAddonInstalled('ALUSAAS') && isCentralDomain()) {
                $this->mapApiRoutes();
                $this->mapWebRoutes();
            }else{
                $this->allApiRoutes();
    ...
    protected function allWebRoutes()
    {
        ...
        if (function_exists('isAddonInstalled') && isAddonInstalled('ALUCOMMITTEE')) {
            ...
            Route::middleware([ 'web', 'auth', 'user', 'is_email_verify', '2fa_verify', 'common'])
                ->as('committee.alumni.')
                ->group(base_path('routes/addon/committee/alumni.php'));
