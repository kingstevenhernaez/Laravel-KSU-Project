<!-- SIDEBAR LOADED: resources/views/layouts/sidebar.blade.php -->
<!-- Sidebar -->
<!-- SIDEBAR_MARKER_123 -->

@php
    // NOTE: In this system, the database uses role = 1 for Admin/Super Admin.
    // Do NOT rely on constants that may not match DB values.
    $roleInt = (int) (auth()->user()->role ?? 0);

    $isAdmin = ($roleInt === 1);

    // Keep alumni/board compatibility (fallback numeric values if constants are not defined).
    $isAlumni = defined('USER_ROLE_ALUMNI') ? ($roleInt === (int) USER_ROLE_ALUMNI) : ($roleInt === 2);
    $isBoardMember = defined('USER_ROLE_BOARD_MEMBER') ? ($roleInt === (int) USER_ROLE_BOARD_MEMBER) : ($roleInt === 4);
@endphp
    ...
    <div class="zSidebar-fixed">
        <ul class="zSidebar-menu" id="sidebarMenu">
            ...
            @if((!isCentralDomain() || !isAddonInstalled('ALUSAAS')) && ($isAdmin || $isAlumni))
                @if($isAlumni || auth()->user()->is_alumni == STATUS_ACTIVE)
                    <li>
            ...
            @if(($isAdmin || $isAlumni))
                ...
                @if($isAlumni || auth()->user()->is_alumni == STATUS_ACTIVE)
                    <li>
                        <a href="#ksu-tools" data-bs-toggle="collapse" role="button"
                           aria-expanded="{{ isset($showKsuTools) ? 'true' : '' }}" aria-controls="ksu-tools"
                           class="d-flex align-items-center cg-10 {{ isset($showKsuTools) ? 'active' : 'collapsed' }}">
                            <div class="d-flex">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M12 2L20 6V12C20 17 16.5 21 12 22C7.5 21 4 17 4 12V6L12 2Z" stroke="white" stroke-opacity="0.7" stroke-width="1.5"/>
                                    <path d="M8 12H16" stroke="white" stroke-opacity="0.7" stroke-width="1.5" stroke-linecap="round"/>
                                    <path d="M12 8V16" stroke="white" stroke-opacity="0.7" stroke-width="1.5" stroke-linecap="round"/>
                                </svg>
                            </div>
                            <span class="">{{ __('KSU Alumni Tools') }}</span>
                        </a>
                        <div class="collapse {{ $showKsuTools ?? '' }}" id="ksu-tools" data-bs-parent="#sidebarMenu">
                            <ul class="zSidebar-submenu">
                                <li><a class="{{ $activeJobHistory ?? '' }}" href="{{ route('job_history.index') }}">{{ __('Job History') }}</a></li>
                                <li><a class="{{ $activeTracerSurveys ?? '' }}" href="{{ route('tracer_surveys.index') }}">{{ __('Tracer Surveys') }}</a></li>
                            </ul>
                        </div>
                    </li>
                @endif
                ...
                @if(!isCentralDomain() || !isAddonInstalled('ALUSAAS'))
                    @if ($isAdmin)
                        ...
                        @can('Manage Alumni')
                            <li>
                                ...
                                <div class="collapse {{ $showAdminAlumni ?? '' }}" id="alumni-admin"
                                    ...
                                    <ul class="zSidebar-submenu">
                                        ...
                                        <li><a class="{{ $activeEnrollmentSync ?? '' }}"
                                               href="{{ route('admin.enrollment_sync.index') }}">{{ __('Enrollment Sync') }}</a>
                                        </li>
                                        <li><a class="{{ $activeTracerSurveysAdmin ?? '' }}"
                                               href="{{ route('admin.tracer_surveys.index') }}">{{ __('Tracer Surveys') }}</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                            ...
                            @php
                                $showAdminAnalytics = request()->routeIs('admin.analytics.*')
                                    || request()->routeIs('admin.job_history_reports.*')
                                    || request()->routeIs('admin.tracer_surveys_reports.*');
                            @endphp

                            <li>
                                <a href="#analytics-admin" data-bs-toggle="collapse" role="button"
                                   aria-expanded="{{ $showAdminAnalytics ? 'true' : 'false' }}"
                                   aria-controls="analytics-admin"
                                   class="d-flex align-items-center cg-10 {{ $showAdminAnalytics ? 'active' : 'collapsed' }}">
                                    <div class="d-flex">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M4 19V5" stroke="white" stroke-opacity="0.7" stroke-width="1.5" stroke-linecap="round"/>
                                            <path d="M4 19H20" stroke="white" stroke-opacity="0.7" stroke-width="1.5" stroke-linecap="round"/>
                                            <path d="M7 15L10 12L13 14L18 9" stroke="white" stroke-opacity="0.7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                    </div>
                                    <span class="">{{ __('Analytics') }}</span>
                                </a>

                                <div class="collapse {{ $showAdminAnalytics ? 'show' : '' }}" id="analytics-admin" data-bs-parent="#sidebarMenu">
                                    <ul class="zSidebar-submenu">
                                        <li class="{{ request()->routeIs('admin.tracer_surveys_reports.analytics') ? 'active' : '' }}">
                                            <a href="{{ route('admin.tracer_surveys_reports.analytics') }}">{{ __('Tracer Survey Analytics') }}</a>
                                        </li>
                                        <li class="{{ request()->routeIs('admin.job_history_reports.analytics') ? 'active' : '' }}">
                                            <a href="{{ route('admin.job_history_reports.analytics') }}">{{ __('Job History Analytics') }}</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
